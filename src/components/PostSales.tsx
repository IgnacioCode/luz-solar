...
export default function PostSales() {
  return (
    <section id="post-venta" className="py-24 bg-slate-100 relative overflow-hidden">
      {/* Decorative Orbs */}
      <div className="absolute top-1/2 left-0 w-[450px] h-[450px] bg-[#19A7CE]/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[350px] h-[350px] bg-[#57C5B6]/5 rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 shadow-xl relative overflow-hidden group">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#19A7CE]/5 to-[#57C5B6]/5 opacity-50 pointer-events-none" />
              
              <div className="relative z-10 space-y-5">
                <div className="flex items-center space-x-3 text-[#19A7CE]">
                  <ShieldCheck className="w-8 h-8 shrink-0 animate-pulse text-[#19A7CE]" />
                  <span className="text-sm font-bold text-[#0B2447] uppercase tracking-wider font-mono">
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-[11px] font-mono font-bold text-slate-400">
                  <div className="flex items-center space-x-1.5 text-slate-500">
                    <HeartHandshake className="w-4 h-4 text-[#57C5B6]" />
                    <span>Soporte 24/7 de Guardia</span>
                  </div>
                  <span className="text-[#F27D26] font-bold">STOCK INMEDIATO</span>
                </div>
              </div>
            </div>
          </div>
            ...
            {postSalesServicesData.map((service) => {
              ...
              return (
                <div
                  ...
                  className="flex flex-col sm:flex-row items-start space-y-4 sm:space-y-0 sm:space-x-5 p-5 sm:p-6 rounded-2xl bg-white border border-slate-200 hover:border-[#19A7CE]/30 hover:shadow-md transition-all duration-300 group hover:bg-white"
                >
                  {/* Icon Frame */}
                  <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-150 text-[#19A7CE] group-hover:text-[#57C5B6] group-hover:border-[#57C5B6]/30 transition-colors shrink-0">
                    {ServiceIcon ? <ServiceIcon className="w-5.5 h-5.5" /> : <Wrench className="w-5.5 h-5.5" />}
                  </div>

                  {/* Text Details */}
                  <div className="space-y-1.5 flex-1">
                    <h3 className="text-sm font-bold text-[#0B2447] group-hover:text-[#19A7CE] transition-colors">
                      {service.title}
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed font-normal">
                      {service.description}
