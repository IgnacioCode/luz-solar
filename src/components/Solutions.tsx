...
export default function Solutions({ onScrollToSection }: SolutionsProps) {
  ...
  return (
    <section id="soluciones" className="py-24 bg-slate-100 relative overflow-hidden">
      {/* Background elements */}
      <div className="absolute top-1/4 right-0 w-[400px] h-[400px] bg-[#19A7CE]/5 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-1/4 left-0 w-[400px] h-[400px] bg-[#57C5B6]/5 rounded-full blur-[100px] pointer-events-none" />
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center space-x-4 p-5 rounded-2xl border text-left transition-all cursor-pointer ${
                    isActive
                      ? 'bg-white border-[#19A7CE] text-[#0B2447] shadow-lg shadow-sky-500/5'
                      : 'bg-white/40 border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-800'
                  }`}
                >
                  <div className={`p-3 rounded-xl border ${
                    isActive 
                      ? 'bg-[#19A7CE]/10 border-[#19A7CE]/20 text-[#19A7CE]' 
            ...
            <div className="p-6 sm:p-10 rounded-3xl bg-white border border-slate-200 shadow-xl space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center space-x-3.5">
                  <div className="p-3.5 rounded-2xl bg-[#19A7CE]/10 border border-[#19A7CE]/20 text-[#19A7CE]">
                    {IconComponent ? <IconComponent className="w-7 h-7" /> : <Home className="w-7 h-7" />}
                  </div>
                  <div>
                    <span className="text-[10px] font-mono tracking-widest text-[#19A7CE] font-bold uppercase">
                      Sector Solicitante
                    </span>
                    <h3 className="text-2xl font-bold text-[#0B2447] mt-0.5">{activeSolution.title}</h3>
                  </div>
                </div>
                ...
                <div className="px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono font-semibold text-slate-700">
                  <span className="text-[#F27D26] block text-[10px] uppercase font-bold tracking-wider mb-0.5">Kit Recomendado:</span>
              ...
              <div className="space-y-3">
                <p className="text-base font-bold text-[#0B2447]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {activeSolution.keyFeatures.map((feat, idx) => (
                    <div key={idx} className="flex items-start space-x-3 text-xs text-slate-600">
                      <CheckCircle className="w-4.5 h-4.5 text-[#57C5B6] shrink-0 mt-0.5" />
              ...
              <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center space-x-2 px-6 py-3 text-xs font-bold rounded-full bg-[#F27D26] text-white hover:bg-orange-600 shadow-md transition-all cursor-pointer"
                >
